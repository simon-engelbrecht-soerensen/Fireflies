   ...
   //          float Blur(float inColor)
    ...
    // 			float sunDotlarge = (pow( sunDot, 2.0 ));
            ...
            { 
            	...
            	float3 sceneColor = SampleSceneColor(IN.screenPos);
            	// return nonLinearDepth;
            	float blackWhite = sceneColor.x + sceneColor.y + sceneColor.z;
            	float depth = smoothstep(0.1, 0.7, saturate(dist2 /_DepthDistance)); 
            	// return depth;
            	// return depth * SimplexNoise(IN.positionOS.xz * 100);
            	// return blackWhite - saturate(dist2 /_DepthDistance);
            	// * SimplexNoise(IN.positionOS.xz * 100)
            	if(saturate(blackWhite) + depth <= 0)
            	{
            		return 0;
            	}
